        ...
        <div className="fixed inset-0 z-[70] bg-black/80 backdrop-blur grid place-items-center p-4" onClick={() => setEditing(null)}>
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-carbon-900 border border-white/15 p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 grid sm:grid-cols-2 gap-3 text-sm">
              ...
              <input type="number" value={editing.torqueNm ?? ''} onChange={(e) => setEditing({ ...editing, torqueNm: +e.target.value })} placeholder="Couple Nm" className="bg-black/50 border border-white/15 px-3 py-2 outline-none" />
              <input type="number" value={editing.weightKg ?? ''} onChange={(e) => setEditing({ ...editing, weightKg: +e.target.value })} placeholder="Poids kg" className="bg-black/50 border border-white/15 px-3 py-2 outline-none" />
              <input type="number" value={editing.seatMm ?? ''} onChange={(e) => setEditing({ ...editing, seatMm: +e.target.value })} placeholder="Selle mm" className="bg-black/50 border border-white/15 px-3 py-2 outline-none" />
              <input type="number" value={editing.fuelL ?? ''} onChange={(e) => setEditing({ ...editing, fuelL: +e.target.value })} placeholder="Réservoir L" className="bg-black/50 border border-white/15 px-3 py-2 outline-none" />
              <input type="number" value={editing.topKmh ?? ''} onChange={(e) => setEditing({ ...editing, topKmh: +e.target.value })} placeholder="V-max km/h" className="bg-black/50 border border-white/15 px-3 py-2 outline-none" />
